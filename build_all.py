...
# 派系名 → kt tag code 對照表（C-016 乾淨輸出，不洩漏派系名到 HTML）
_PIE_TO_KT = {
    '拆解':         'kt1',
    '個人化諮詢':   'kt2',
    '嗆辣':         'kt3',
...
def _pie_to_tag_codes(pie: str, cta: str) -> str:
    """把 pie（派系名）和 cta 轉成 kt* code，不洩漏中文派系名"""
    codes = []
    p = _PIE_TO_KT.get(pie, '')
    if p:
        codes.append(p)
    c = _PIE_TO_KT.get(cta, '')
    if c and c not in codes:
        codes.append(c)
    return ' '.join(codes) if codes else 'kt8'
...
def kenny_article(num, title, pie, platforms, cta, summary, timeline,
    ...
    # C-016 乾淨輸出：data-tags 用 kt* code，不洩漏派系名
    tag_codes = _pie_to_tag_codes(pie, cta)

    return (
        '<div class="script-card" data-tags="' + esc_attr(tag_codes) + '" data-id="' + cid + '"' + cap_attr + hashtag_attr + '>\n'
        '  <div class="card-top">\n'
        '    <div class="card-num">#' + str(num).zfill(2) + '</div>\n'
        '    <div class="card-title-block">\n'
        '      <div class="card-title">' + title_e + '</div>\n'
        '      <div class="card-meta"><span>60 秒</span><span>' + batch_e + '</span></div>\n' +
...
def group_v2(label, en, gid, cards, collapsed=True):
    # C-016: label（派系名）不輸出到 HTML，只保留 en
    inner = '\n'.join(cards)
    cnt = str(len(cards))
...
k05_07 = kenny_article(7,'30 歲還不買房的人，我幫你說一句','嗆辣',['TikTok', 'IG Reels'],'個人化諮詢',
    ...
    hashtag=['#30歲', '#買房', '#首購族', '#高雄買房', '#kennywangbb', '#仲豪', '#高雄房仲', '#世代觀察', '#正能量', '#30歲視角'])
...
# 12 買房前，你有查過凶宅嗎
k05_12_card = kenny_article(12,'買房前，你有查過凶宅嗎','圖卡部',['IG Reels', 'TikTok'],'留言取得查詢方法',
...
# ============================================================
# Assemble v4 groups (第05批 only — 第03批 archived 2026-05-21)
# 第05批 派系分組（R3 更新後）：
...
# 嗆辣派（05批 1部）
g_pain = group_v2('嗆辣派', 'Spicy', 2,
    [k05_07])

# 自嘲反差（05批 3部）
g_story = group_v2('自嘲反差', 'Self-Irony', 3,
    [k05_02, k05_10, k05_11])
...
# Patch: C-016 乾淨 chip（不洩漏派系名到 data-filter 或 chip 文字）
nc = nc.replace('>釣魚部</div>', '>圖卡</div>')
nc = nc.replace('data-filter="釣魚"', 'data-filter="kt4"')
nc = nc.replace('data-filter="圖卡部"', 'data-filter="kt4"')
# 修正 chip tag-row: C-016 乾淨 chip（kt 碼不含派系名）
# 用 regex 替換整個 tag-row 塊（含所有內嵌 chip div）
_new_chip_row = ('<div class="tag-row">'
    + chr(10) + '      <div class="chip active" data-filter="all">全部</div>'
    + chr(10) + '      <div class="chip" data-filter="kt3">反擊</div>'
    + chr(10) + '      <div class="chip" data-filter="kt1">拆解</div>'
    + chr(10) + '      <div class="chip" data-filter="kt4">圖卡</div>'
    + chr(10) + '      <div class="chip" data-filter="kt6">觀察</div>'
    + chr(10) + '      <div class="chip" data-filter="kt2">諮詢</div>'
    + chr(10) + '      <div class="chip" data-filter="kt7">自嘲</div>'
    + chr(10) + '    </div>')
nc = re.sub(r'<div class="tag-row">.*?</div>', _new_chip_row, nc, count=1, flags=re.DOTALL)
# C-016: 移除 tag-row 之後殘留的裸 chip blocks（template 累積殘留）
# 策略：找 tag-row 結束位置，再刪其後所有裸 chip div 群（不帶 tag-row wrapper）
_tr_end = nc.find(_new_chip_row)
if _tr_end >= 0:
    _tr_end += len(_new_chip_row)
    # 從 tag-row 結束後開始，刪除所有緊跟的裸 chip divs（以 </div>\n 作為 chunk 結束）
    nc = nc[:_tr_end] + re.sub(
        r'^(\s*<div class="chip"[^>]*>[^<]*</div>\s*\n)+',
        '',
        nc[_tr_end:]
    )
...
assert len(arts) >= 13, f'Expected >= 13 cards (05批13), got {len(arts)}'
# verify 圖卡部 card exists (data-id k12 is 圖卡部 card)
assert 'data-id="k12"' in nc, 'Missing k12 card'
