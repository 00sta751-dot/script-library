...
def sc_article(num, title, pie, platforms, cta, scene, timeline, batch=None,
    ...
    派系_tags = ''  # C-016: 不輸出派系名到 HTML
    cta_tag = ''  # C-016: 不輸出 cta 可見文字（避免派系名洩漏到 HTML）
    tl_html = ''
...
def gp_group_v2(key, label, arts, collapsed=True):
    # C-016: label（派系名）不輸出到 HTML，只保留 key
    inner = '\n'.join(arts)
    cnt = str(len(arts))
...
b02_06 = b2(6,'吵架後你說的第一句話，決定了你們','圖卡部',['IG Reels','FB Reels'],'留言和好取圖卡',
    '兩人面對面，事後回憶狀態',
...
b02_18 = b2(18,'開瓶蓋那件小事','直球情侶版',['IG Reels','Threads'],'（無 CTA）',
    '廚房或餐桌，簡單日常',
...
b03_01 = sc_article(101,'他帶看一整天，進門第一件事','故事戲劇派',['IG Reels','FB Reels'],'個人化諮詢',
    ...
    [('0-3秒 Hook','叭噗（推門進來，扶著門）：累死了。字幕：他帶看一整天，進門第一句',''),
     ...
     ('12-25秒','小C（從廚房探頭）：我有留飯。先去換衣服。字幕：留了飯，催他洗。','','19 年，她看懂了他的累⋯⋯'),
...
b03_02 = sc_article(102,'不過夜規定這件事，我們的答案','人間觀察派',['IG Reels','TikTok'],'個人化諮詢',
    ...
    [('0-3秒 Hook','小C（直視鏡頭）：我跟他在一起的時候沒有不過夜規定這件事。字幕：沒有不過夜規定',''),
     ...
     ('25-40秒','叭噗：差在哪，差在你對那個人有沒有底氣。字幕：差在有沒有底氣。','','靠的不是規定，是底氣——這個說法！'),
...
b03_04_card = sc_article(104,'19 年了，你們怎麼不越來越陌生？','圖卡部',['IG Reels','FB Reels'],'留言不陌生取圖卡',
    '兩人同框，室內，叭噗和小C 對鏡頭說話，認真表情',
    [('0-3秒 Hook','叭噗（對鏡頭）：19 年，常有人問——你們不會越來越陌生嗎？字幕：19 年，不怕陌生？','','19 年耶——真的不會嗎？！'),
     ('3-12秒','小C：老實說，這是長期感情裡最真實的問題。字幕：這是長期感情最真實的問題',''),
     ('12-25秒','叭噗：我們也沒有答案說我們「從來不陌生」——我們中間有段時間，兩個人都像在自己的世界裡。字幕：中間也有那段時間',''),
     ('25-40秒','小C：但我們做了幾件事，讓那個「各自在自己世界」不變成陌生。叭噗：想知道是什麼嗎？字幕：讓各自世界不變陌生的方法。','','我要知道！！'),
...
b03_07 = sc_article(107,'旅遊這件事，我們從來沒有完全意見一致','自嘲反差派',['IG Reels','TikTok'],'個人化諮詢',
    '兩人同框，底片相機畫面或家中，輕鬆表情',
    [('0-3秒 Hook','叭噗：我們旅遊，19 年沒有一次完全一致過。字幕：旅遊計畫從來沒全一致。','','哈哈哈哈哈這不就是我們！！'),
     ('3-12秒','小C：我要曬太陽，他要走路。叭噗：我要走路，她要待在旁邊的咖啡廳。小C：對。字幕：他走路，她進咖啡廳',''),
     ('12-25秒','叭噗：去北海道，我拍兩小時，她在民宿看書。晚餐一起吃。小C：完美。字幕：各玩各的，晚餐聚',''),
     ('25-40秒','叭噗：各自玩，再一起吃飯，比全黏在一起更好玩。字幕：各自玩，聚一起，更有趣。','','這個模式聽起來很解放⋯⋯我要跟我另一半講！'),
...
b03_08 = sc_article(108,'有一種人，19 年不分的原因你想不到','拆解派',['IG Reels','FB Reels'],'個人化諮詢',
    ...
    [('0-3秒 Hook','小C：19 年不分的原因，你猜不到。字幕：他為什麼一直都在。','','什麼原因？！'),
     ('3-12秒','小C：不是相似，是相反。字幕：其實是互補，不是匹配',''),
     ('12-25秒','叭噗：我出去她在家。我安靜她說話。各做各的。小C：晚餐一定一起吃。字幕：各自的世界，晚餐聚',''),
     ('25-40秒','小C：有人說要非常契合——其實是互補。你一方，他另一方，加起來才完整。字幕：互補，不是完全合拍。','','互補才完整——這個說法我沒想過！'),
...
b03_09 = sc_article(109,'他說他不在乎，但他記了 4 年','故事戲劇派',['IG Reels','TikTok'],'個人化諮詢',
    ...
    [('0-3秒 Hook','小C：他說不在乎。記了 4 年。字幕：嘴上不在乎，心裡記著。','','什麼事？！'),
     ('3-12秒','小C：我說想去沖繩，他說現在沒辦法。我說沒關係。就結束了。字幕：沒在一起時說的話',''),
     ('12-25秒','小C：4 年後他說，你不是想去沖繩？我說對。他說那規劃一下。就去了。字幕：4 年後才提起，還真的去',''),
     ('25-40秒','叭噗：只是記在腦袋裡。沒什麼大事。小C：你說不在乎。記了 4 年。字幕：說不在乎，真的記著。','','這就是長情⋯⋯他的愛在行動裡⋯⋯'),
...
b03_10 = sc_article(110,'我嫁給他之前不知道的一件事','人間觀察派',['IG Reels','FB Reels'],'個人化諮詢',
    ...
    [('0-3秒 Hook','小C：嫁給他之前，有件事我不知道。字幕：婚前沒發現的事。','','什麼事？！'),
     ('3-12秒','小C：他吃得很慢。叭噗：我在享受。小C：吃飯他吃第三口，我快吃完。字幕：他超級慢，我卡著',''),
     ('12-25秒','字幕：不會吵？小C：一開始會等急。後來我帶書，他吃完我們再走。字幕：學著等，現在沒問題',''),
     ('25-40秒','叭噗：她也有我不知道的。小C：什麼？叭噗：妳睡前一定要聊天。小C：對啦。字幕：她睡前必聊天。','','互相揭露⋯⋯婚前不知道，婚後接受⋯⋯這才是生活！'),
...
b03_11 = sc_article(111,'「我愛你」這三個字，我們好像很少說','人間觀察派',['IG Reels','Threads'],'個人化諮詢',
    '兩人同框，家中，認真但輕鬆的氛圍',
    [('0-3秒 Hook','小C：19 年，我們好像很少說「我愛你」。字幕：19 年沒說過幾次。','','不浪漫欸——'),
     ('3-12秒','小C：上週他幫我換水管。漏水很久。字幕：他去換了漏了很久的水管',''),
     ('12-25秒','叭噗：我愛你三個字讓我覺得假。做不就好。小C：他邏輯這樣。我一開始也覺得少了什麼。字幕：他覺得三個字作假，說還不如做',''),
     ('25-40秒','小C：後來我想，如果天天說愛你，水管漏三個月不理——我選哪種？字幕：說愛你 vs 換水管，我要哪種。','','毒但真！！！'),
...
b03_12 = sc_article(112,'某一年我突然明白，陪伴的意思','直球情侶版',['IG Reels','Threads'],'（無 CTA）',
    '兩人同框，底片相機質感畫面或家中，氣氛溫暖',
    [('0-3秒 Hook','小C（輕聲，對鏡頭）：某一年，我才明白陪伴的意思。字幕：那一年我才懂什麼是陪伴。','','說下去⋯⋯'),
     ('3-12秒','小C：不是每秒在一起。是你煩惱時，有個人可以講。字幕：陪伴就是有人聽你',''),
     ('12-25秒','叭噗：我們不是天天聊。各自忙，各自靜。但我知道她在。字幕：她在，就夠了',''),
     ('25-40秒','小C：陪伴不是熱鬧。是最安靜的時候，他還在。字幕：陪伴就是這個。','','最溫柔的⋯⋯'),
...
# ============================================================
# 第 04 批 articles (試水批 13 parts — yaml-driven 動態分流)
# ============================================================
import sys as _sys
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
if _THIS_DIR not in _sys.path:
    _sys.path.insert(0, _THIS_DIR)
from yaml_to_sc import load_yaml_articles, yaml_to_sc_kwargs

YAML_DIR_04 = r'C:\Users\00sta\Documents\Claude\Projects\短影音系統\L2_業主層\情侶_叭噗_小C\02_腳本生產\第04批_試水批_2026-05-21'

# 動態讀 yaml 派系欄位 → 分流到各派系 bucket（yaml-driven，禁 hardcode if-elif）
_b04_by_pie = {}  # dict[派系名 → list[article HTML]]
for _idx, _ydata in enumerate(load_yaml_articles(YAML_DIR_04, expected_count=EXPECTED_COUNT_04), start=1):
    _pie = _ydata.get('派系', '未分類')
    _kwargs = yaml_to_sc_kwargs(_ydata, num=400 + _idx)
    _kwargs['batch'] = BATCH_04
    _art = sc_article(**_kwargs)
    _b04_by_pie.setdefault(_pie, []).append(_art)

# 印分流統計供驗證
_total = sum(len(v) for v in _b04_by_pie.values())
print(f'第04批 articles built OK ({_total} parts):')
for _pie, _arts in sorted(_b04_by_pie.items()):
...
assert not _b04_unused, (
    f'❌ 04 批未消費派系 (silent drop 風險): {_b04_unused}\n'
    f'   - 編劇若新增派系必更新 B04_ALLOWED_PIES + 對應 group 合入邏輯\n'
    f'   - 或修正 yaml 派系欄位 typo'
)
assert _b04_guard_total == EXPECTED_COUNT_04, (
    f'❌ 04 批合入部數 {_b04_guard_total} ≠ {EXPECTED_COUNT_04}（13）\n'
...
.mirror{
  ...
  border:1px dashed rgba(224,64,251,.4);
}
"""
...
MIRROR_CSS_MARKER = '/* ========== .mirror 藏鏡人泡泡 (build_bappu.py v2.1) ========== */'
MIRROR_CSS = """
/* ========== .mirror 藏鏡人泡泡 (build_bappu.py v2.1) ========== */
.mirror{
  margin-top:6px;display:inline-block;
  font-size:11.5px;
  background:rgba(224,64,251,.12);color:#e040fb;
...
function toggleGroup(grp){grp.classList.toggle('op');grp.classList.toggle('collapsed');}
...
if V2_JS_MARKER in nc:
    # Replace existing patch entirely to pick up any updates to V2_JS template
    marker_start = nc.index(V2_JS_MARKER)
    # Find end: next </script> after marker
    script_close = nc.find('</script>', marker_start)
    if script_close >= 0:
        nc = nc[:marker_start] + V2_JS.strip() + '\n' + nc[script_close:]
    else:
        nc = nc[:marker_start] + V2_JS.strip()
    print('V2 JS patch replaced (updated)')
else:
...
# C-016 乾淨：移除殘留的派系名 HTML comment（舊版 build 遺留在 main-wrap 前的 comment）
import re as _re
nc = _re.sub(r'\n?<!-- (?!group-)(?![\#])[^-]{1,40} -->', '', nc)
