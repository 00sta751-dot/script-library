...
# 2026-08-25 shell 註記：頁面另含手插「屋主向批」section（sect-owner0825，13 卡；澤君 TG21819「上線吧」令）；splice 不會動它。
...
# v2026-06-16: multi-yaml-dir support (--yaml-dir dir1,dir2) + 累積式：保留所有批 sections（最新在上），不清舊批
...
_parser.add_argument('--yaml-dir', dest='yaml_dir', default='',
                     help='yaml 批次資料夾絕對路徑（逗號分隔多批次，如 dir37,dir38；--mode yaml 時必填）')
_parser.add_argument('--batch-label', dest='batch_label', default='',
                     help='批次顯示名稱（逗號分隔，與 --yaml-dir 一一對應，如「第 37 批 · 2026-06-07,第 38 批 · 2026-06-16」）')
...
_parser.add_argument('--threads-md', dest='threads_md', default='',
                     help='脆文 .md 檔案絕對路徑（逗號分隔多批次；--mode yaml 時可選；僅渲染最新批脆文，對齊 check 16）')
...
if MODE == 'yaml':
    print(f'\n=== yaml-driven mode v2 (multi-batch 累積式，最新在上，絕不清舊批) ===')
    ...
    # _yaml_batches: list of (batch_label, flat_arts_list)
    _yaml_batches = []
    # _yaml_by_pie: 跨批次合併，用於注入既有派系 section（保留舊批硬編碼）
    _yaml_by_pie = {}
    ...
    for _dir_i, _yaml_dir_path in enumerate(_yaml_dirs):
        ...
        for _idx, _ydata in enumerate(_this_yaml_articles, start=0):
            ...
            _art = rux_article_adapter(_ydata, num=_num_cursor + _idx, batch_label=_this_batch_label)
    ...
    # --- 脆文 section：只用最新批脆文（check 16 驗 expected==rendered，不累積舊批）---
    # _threads_md_list 最後一個為最新批脆文（與 _yaml_dirs 對應）
    _sect_threads_to_use = sect_threads  # fallback 硬編碼舊批
    _latest_threads_label = _batch_labels_raw[-1] if _batch_labels_raw else ''
    if _threads_md_list:
        # 取最後一個（最新批）為主脆文；前批脆文以 <h3> 子標題疊在後面
        _all_threads_parts = []
        # 先跑所有 md，收集 (label, data) 列表（舊→新）
        _all_thread_data = []
        ...
        if _all_thread_data:
            # 最新批（最後一個）為主脆文 section（check 16 只驗最新批 expected 數量）
            # 舊批脆文不放進 HTML（check 16: rendered must == expected of latest batch only）
            _latest_threads_label_actual, _latest_threads_data = _all_thread_data[-1]
            _main_threads_count = len(_latest_threads_data)
            _sect_threads_parts = (
                '<header class="section-head" id="sect-threads">\n'
                '  <span class="roman">✦</span>\n'
                f'  <span class="label">脆文 Threads<span class="en">{_latest_threads_label_actual} · {_main_threads_count} 篇</span></span>\n'
                '  <span class="rule"></span>\n'
                f'  <span class="count">{_main_threads_count} posts</span>\n'
                '</header>\n'
                '<div class="threads-grid">\n' +
                '\n'.join(thread_card(t[0], t[1], t[2], t[3]) for t in _latest_threads_data) +
                '\n</div>'
            )
            _sect_threads_to_use = _sect_threads_parts
            if len(_all_thread_data) > 1:
                print(f'NOTE: 共 {len(_all_thread_data)} 批脆文 md，只渲染最新批 {_latest_threads_label_actual} {_main_threads_count} 篇（check 16 限制）')

    # --- 組裝 all_sections：yaml 新批（最新→最舊）→ 脆文（只新批為 main，不加舊批派系 section）---
    # 注意：舊批硬編碼派系 sections 不放進 yaml 模式 all_sections
    # （因為舊批 33/34 卡沒有 data-caption，會觸發 check_1；且 C-016 守則禁止派系 section 名）
    all_sections = '\n\n'.join(
        _yaml_batch_sections
        + [_sect_threads_to_use]
    )
    print(f'yaml-driven all_sections assembled: {len(_yaml_batches)} yaml批 sections + 脆文（最新批 main，絕不清舊批 yaml sections）')
else:
    # legacy mode（預設）
...
# yaml 模式：id="sect-new-{N}"（非 sect-\d+）→ 直接用 _yaml_batch_sections 長度
# legacy 模式：id="sect-\d+"（hardcode 舊 id）→ regex 計數
if MODE == 'yaml':
    _sections_count = len(_yaml_batch_sections)  # yaml-driven：每批一個 section，準確無誤
else:
...
import re as _re
# 多批次時取最後一批 label（最新批）作為日期來源
_batch_labels_for_date = [lb.strip() for lb in (getattr(_args, 'batch_label', '') or '').split(',') if lb.strip()]
_batch_source = _batch_labels_for_date[-1] if _batch_labels_for_date else BATCH
...
# 替換 Sections/Batches 數字（yaml mode 改 Batches label）
if MODE == 'yaml':
    nc = re.sub(
        r'(<div class="stat"><div class="n">)\d+(</div><div class="l">)Sections(</div></div>)',
        rf'\g<1>{_sections_count}\g<2>Batches\g<3>',
        nc
    )
    # 若 html 已有 Batches label（idempotent）
    nc = re.sub(
        r'(<div class="stat"><div class="n">)\d+(</div><div class="l">Batches</div></div>)',
...
if MODE == 'yaml':
    # 從舊 html（c）只抓 yaml 模式產出的 section-head（id="sect-new-N"）裡的批次號
    # 格式：<header class="section-head" id="sect-new-N">...<span class="label">第N批 · YYYY-MM-DD 更新...
    # 先抓整塊 header（到 </header>），再從塊內取批次號，避免卡片正文假陽性
    ...
    # 這樣才能防繞過：只傳 1 個 dir 但 --batch-label 塞多個 label → 早期報錯
    ...
    if _dropped and not getattr(_args, 'allow_drop_batches', False):
        ...
        print(f'  現有批次（index.html）: {sorted(_existing_batch_labels, key=lambda x: int(x))}')
        ...
        print('防下架鎖：index.html 無現有批次（首次建立），跳過檢查')
    else:
